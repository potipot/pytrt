...
def main(verbose: bool = False, workspace: int = 2, precision: str = "fp16"):
    ...
    for image_path in images_dir.iterdir():
        ...
        logger.debug(f"Encoder inference time: {encoder_inference.inference_time}")
        feature_map, *_ = encoder_output
        decoders_output = decoders_inference.infer(feature_map)
        logger.debug(f"Decoders inference time: {decoders_inference.inference_time}")
        logger.info(f"Total inference time: {encoder_inference.inference_time+decoders_inference.inference_time}")
